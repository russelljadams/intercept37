...
export default function Repeater() {
  ...
  return (
    <div className="flex gap-4 h-full animate-slide-in">
      {/* Main area */}
      <div className="flex-1 flex flex-col gap-4 min-w-0">
        {/* Request Builder */}
        <div className="bg-alien-panel border border-alien-border rounded-lg p-4 space-y-3">
        ...
        {/* Response */}
        <div className="flex-1 bg-alien-panel border border-alien-border rounded-lg p-4 overflow-auto">
          <h3 className="text-alien-green text-xs uppercase tracking-[0.15em] mb-3">Response</h3>
          {!response && !sending ? (
            <div className="text-alien-text-dim text-xs text-center py-12">
              Build a request above and click SEND to see the response here.
            </div>
          ) : sending ? (
            <div className="text-alien-green text-xs animate-flicker text-center py-12">
              Transmitting...
            </div>
          ) : response ? (
            <div className="space-y-3">
              ...
              <div>
                ...
                <pre className="bg-alien-black border border-alien-border rounded p-3 text-xs text-alien-text-dim whitespace-pre-wrap max-h-32 overflow-auto">
                  {Object.entries(response.headers).map(([k, v]) => `${k}: ${v}`).join('\n') || '(none)'}
                </pre>
              </div>
              <div>
                <h4 className="text-alien-text-dim text-[10px] uppercase tracking-wider mb-1">Body</h4>
                <pre className="bg-alien-black border border-alien-border rounded p-3 text-xs text-alien-cyan whitespace-pre-wrap overflow-auto max-h-96">
      ...
      {/* History Sidebar */}
      <div className="w-64 flex-shrink-0 bg-alien-panel border border-alien-border rounded-lg p-3 overflow-auto">
        <h3 className="text-alien-green text-xs uppercase tracking-[0.15em] mb-3">History</h3>
        {history.length === 0 ? (
          <div className="text-alien-text-dim text-[10px] text-center py-4">No requests sent yet</div>
        ) : (
