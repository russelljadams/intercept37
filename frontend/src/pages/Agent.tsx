...
import { useAgentChat, type ChatMessage, type ToolCall, type ContentBlock } from '../hooks/useAgentChat';
...
function BlockRenderer({ block, isLast, isStreaming }: { block: ContentBlock; isLast: boolean; isStreaming: boolean }) {
  if (block.type === 'text' && block.text) {
    ...
    return (
      <div className="bg-alien-panel border border-alien-border rounded-lg px-3 py-2 sm:px-4 sm:py-3 my-1.5">
...
export default function Agent() {
  const { messages, isStreaming, sendMessage, clearSession } = useAgentChat();
