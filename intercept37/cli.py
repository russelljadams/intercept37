...
import click
import uvicorn
from intercept37.db import init_db

logging.basicConfig(
    level=logging.INFO,
    format="%(asctime)s [%(name)s] %(levelname)s: %(message)s",
)
logger = logging.getLogger("intercept37")


@click.group()
def main():
    """intercept37 — pentest suite for humans and LLMs."""
    pass


@main.command()
@click.option("--proxy-port", default=8080, help="Proxy listen port")
@click.option("--api-port", default=1337, help="API/dashboard port")
@click.option("--host", default="0.0.0.0", help="Listen host")
def start(proxy_port: int, api_port: int, host: str):
    """Start intercept37 (proxy + API server)."""
    from intercept37.proxy.engine import ProxyEngine
    from intercept37.api.server import app, proxy_listener

    init_db()
    logger.info("intercept37 v0.1.0 starting up...")
...
@main.command()
@click.option("--port", default=1337, help="API port")
@click.option("--host", default="0.0.0.0", help="Listen host")
def api_only(port: int, host: str):
    """Start only the API server (no proxy)."""
    from intercept37.api.server import app
    init_db()
